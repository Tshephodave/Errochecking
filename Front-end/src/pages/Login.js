...
const Login = ({ setUser }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://marketing-server-9i2h.onrender.com/user/login', formData);
      const token = response.data.token;
      localStorage.setItem('token', token);

      const { userId } = jwtDecode(token);
      const userResponse = await axios.get(`https://marketing-server-9i2h.onrender.com/user/${userId}`, {
        headers: {
